...
def load_vocab_with_rationales(path: str) -> Tuple[List[str], List[Dict[str, str]]]:
    with open(path, "r", encoding="utf-8") as f:
        payload = json.load(f)

    labels = payload.get("labels")
    if not isinstance(labels, list):
        raise ValueError('vocab JSON must be shaped like: {"labels": [{"label":"...","rationale":"..."}]}')

    entries: List[Dict[str, str]] = []
    label_names: List[str] = []
    for idx, item in enumerate(labels):
        if not isinstance(item, dict):
            raise ValueError(f"Label entry at index {idx} must be an object.")
        label = item.get("label")
        rationale = item.get("rationale")
        if not isinstance(label, str) or not isinstance(rationale, str):
            raise ValueError(f"Label entry at index {idx} must have string 'label' and 'rationale'.")
        entries.append({"label": label, "rationale": " ".join(rationale.split())})
        label_names.append(label)

    if not label_names:
        raise ValueError("vocab JSON must include at least one label.")

    return label_names, entries
...
def classify_prompt(
    ...
    label_guidance: List[Dict[str, str]],
) -> str:
    labels_bullets = "\n".join(f"- {x}" for x in allowed_labels)
    guidance_bullets = "\n".join(
        f"- {item['label']}: {item['rationale']}" for item in label_guidance
    )
...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    ap.add_argument(
        "--vocab",
        required=True,
        help='Path to vocab JSON like {"labels":[{"label":"...","rationale":"..."}]}',
    )
...
def main(argv: List[str]) -> int:
    ...
    labels, label_guidance = load_vocab_with_rationales(args.vocab)
