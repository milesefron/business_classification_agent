...
def parse_args(argv: List[str]) -> argparse.Namespace:
    ...
    ap.add_argument("infile", help="path/to/an/input/csv file")
...
def main(argv: List[str]) -> int:
    ...
    #read in csv file
    #df = pd.read_csv("data/pending_employers_review.csv", encoding="latin1")
    ...
    input = args.infile
    print(f"Reading companies file csv at: f{input}")

    client = OpenAI()

    #read in csv file - sample
    df = pd.read_csv("data/sample_pending_employers.csv", encoding="latin1")

    #print out company names - diagnostic
    first_col = df.iloc[:, 0]

    for company in first_col:
        company_pristine = company
        ...
        payload['input_name'] = company_pristine

        OUT_DIR = "data/output/"
        OUT_FILE = OUT_DIR + company.strip().replace(' ', '_') + ".json"
        print(OUT_FILE)
